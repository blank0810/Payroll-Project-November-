...
namespace Payroll_Project2.Forms.Mayor.Travel_Order_Requests
{
    public partial class travelRequestsUC : UserControl
    {
        ...
        public travelRequestsUC(int userId, MayorDashboard parent, string userDepartment)
        {
            InitializeComponent();
            _userId = userId;
            _parent = parent;
        ...
        private async Task<DataTable> GetTravelRequestList(string department, int offset, int recordPerPage)
        {
            try
            {
                DataTable list = await formRequestClass.GetTravelRequest(department, offset, recordPerPage);
        ...
        private async Task<int> GetTotalRecords(string department)
        {
            try
            {
                int total = await mayorDashboard.GetNumberOfTravelRequest(department);
                return total;
            }
            catch (SqlException sql) { throw sql; } catch (Exception ex) { throw ex; }
        }
        ...
        public async Task DisplayRequest(int userId, string department)
        {
            try
            {
                ...
                DataTable list = await GetTravelRequestList(department, offset, recordPerPage);

                if(list != null && list.Rows.Count > 0)
                {
                    requestDataUC[] travelList = new requestDataUC[list.Rows.Count];

                    for (int i = 0; i < list.Rows.Count; i++)
                    {
                        travelList[i] = new requestDataUC(userId, this, department);
                        DataRow row = list.Rows[i];

                        ParseAndAssignInt(row, "employeeId", value => travelList[i].EmployeeId = value, 0);
                        AssignValueIfNotEmpty(row, "employeeName", value => travelList[i].EmployeeName = value, "-------");
                        ParseAndAssignInt(row, "orderControlNumber", value => travelList[i].ControlNumber = value, 0);
                        ParseAndAssignDateTime(row, "dateFiled", value => travelList[i].DateFiled = value, "-------");
                        ParseAndAssignDateTime(row, "dateDeparture", value => travelList[i].DateDeparture = value, "--------");
                        
                        bool.TryParse(row["isNoted"]?.ToString(), out bool isNoted);
                        if(isNoted && !string.IsNullOrEmpty(row["isNoted"].ToString()))
                        {
                            travelList[i].IsNoteNull = false;
                        }
                        else
                        {
                            travelList[i].IsNoteNull = true;
                        }

                        requestListPanel.Controls.Add(travelList[i]);
                    }
                }
            }
            catch (SqlException sql)
        ...
        private async void travelRequestsUC_Load(object sender, EventArgs e)
        {
