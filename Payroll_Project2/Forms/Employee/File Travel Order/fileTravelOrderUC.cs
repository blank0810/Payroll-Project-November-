...
using Payroll_Project2.Forms.Employee.File_Travel_Order.File_Travel_Order_sub_user_control;
using System;
    ...
    public partial class fileTravelOrderUC : UserControl
    {
        ...
        private async Task DisplayRecentTravel(int employeeId, int month, int year)
        {
            try
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    travelOrderDataUC travelOrder = new travelOrderDataUC();
                    recentListPanel.Controls.Add(travelOrder);
                }
        ...
        private async void submitBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                await _parent.DisplayFileTravelOrder();
            }
            catch (SqlException sql)
            {
                ErrorMessages(sql.Message, "SQl Error");
